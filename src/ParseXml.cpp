...
Http_ParseXML::Http_ParseXML(){ 
	this->Loaded = false; //ly obj_new
	//this->ObjXml = new ParseXml_Doc;
}

Http_ParseXML::Http_ParseXML(const char *file_path) {
	this->Loaded = false; //ly obj_new
	if (this->LoadFile(file_path) == LOAD_XML_ERROR)
		cout << "LOAD XML file error:" << file_path << endl;
}
Http_ParseXML::~Http_ParseXML(void){
...
bool Http_ParseXML::LoadSuccess() {
	//ly obj_new
	return this->Loaded; 
}
